...
import com.thinkme.utils.concurrent.threadpool.ThreadPoolBuilder;
...
public class ThreadDumpperTest {
	...
	public void test() throws InterruptedException {
		...
		CountDownLatch countDownLatch= com.thinkme.utils.concurrent.ConcurrentTools.countDownLatch(10);
		for(int i=0;i<10;i++){
			executor.execute(new LongRunTask(countDownLatch));
		}
		countDownLatch.await();
		
		com.thinkme.utils.concurrent.ThreadDumpper dumpper = new com.thinkme.utils.concurrent.ThreadDumpper();
		dumpper.threadDumpIfNeed();

		LogbackListAppender appender = new LogbackListAppender();
		appender.addToLogger(com.thinkme.utils.concurrent.ThreadDumpper.class);
